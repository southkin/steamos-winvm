...
create_container() {
  ...
  local args=(--name "$CONTAINER_NAME" --image "$CONTAINER_IMAGE" --home "$CONTAINER_HOME" --volume "$VM_ROOT_DIR:$VM_ROOT_DIR:rw" --additional-packages "$init_packages" --init-hooks "$init_hooks")
...
install_quickemu() {
  ...
  dbx_bash 'command -v quickemu >/dev/null 2>&1 && command -v quickget >/dev/null 2>&1' || die "Quickemu is missing from the distrobox. Recreate the container."
}
