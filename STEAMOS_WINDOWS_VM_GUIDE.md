...
`~/Downloads` 아래 ISO 파일을 번호로 골라 바로 `import-iso`에 넘기고 싶다면 `./pick-windows-iso.sh` 를 쓸 수 있습니다. `RUN_AFTER_IMPORT=1 ./pick-windows-iso.sh` 로 실행하면 가져온 뒤 바로 VM 실행까지 이어집니다.
...
처음 만든 컨테이너는 rootless/rootful 여부와 관계없이 첫 `enter`에서 초기 설정을 마칠 수 있습니다. 스크립트가 컨테이너 쉘을 열면 프롬프트가 뜬 뒤 `exit`로 나오면 다음 단계가 이어집니다.
